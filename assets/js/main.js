...
(function () {
  ...
  function initEventTracking() {
    document.addEventListener('click', function (e) {
      // 1. App Store Tracking
      var appButton = e.target.closest('.app-store-button');
      if (appButton) {
        var appCard = appButton.closest('.android-app-card');
        if (appCard) {
          var packageEl = appCard.querySelector('.package');
          var titleEl = appCard.querySelector('.android-app-title');
          var img = appButton.querySelector('img');
          var store = 'Unknown Store';
          if (img && img.src) {
            var src = img.src.toLowerCase();
            if (src.indexOf('googleplay') !== -1) store = 'Google Play';
            else if (src.indexOf('appgallery') !== -1) store = 'Huawei AppGallery';
            else if (src.indexOf('galaxystore') !== -1) store = 'Samsung Galaxy Store';
            else if (src.indexOf('mistore') !== -1) store = 'Xiaomi Mi Store';
            else if (src.indexOf('amazon') !== -1) store = 'Amazon Appstore';
            else if (src.indexOf('appstore') !== -1) store = 'Apple App Store';
          }
          if (window.gtag) {
            console.log('✅ GA4 Custom Event Tracked: app_store_click ->', store, '|', titleEl ? titleEl.textContent.trim() : 'unknown');
            gtag('event', 'app_store_click', {
              app_package: packageEl ? packageEl.textContent.trim() : 'unknown',
              app_name: titleEl ? titleEl.textContent.trim() : 'unknown',
              app_store: store
            });
          }
        }
      ...
      // 2. Web Project Tracking
      var webCard = e.target.closest('.portfolio-card');
      if (webCard && !webCard.classList.contains('android-app-card')) {
        var webLink = e.target.closest('a');
        if (webLink && webCard.contains(webLink)) {
          var webTitle = webCard.querySelector('h3');
          if (window.gtag) {
            console.log('✅ GA4 Custom Event Tracked: web_project_click ->', webTitle ? webTitle.textContent.trim() : 'unknown');
            gtag('event', 'web_project_click', {
              web_name: webTitle ? webTitle.textContent.trim() : 'unknown',
              web_url: webLink.href
            });
          }
        }
        return;
      ...
      var certCard = e.target.closest('.cert-list-card');
      if (certCard) {
        var certLink = e.target.closest('a');
        if (certLink && certCard.contains(certLink)) {
          var certTitle = certCard.querySelector('h3');
          var certIssuer = certCard.querySelector('.cert-meta');
          if (window.gtag) {
            console.log('✅ GA4 Custom Event Tracked: certificate_click ->', certTitle ? certTitle.textContent.trim() : 'unknown');
            gtag('event', 'certificate_click', {
              certificate_name: certTitle ? certTitle.textContent.trim() : 'unknown',
              certificate_issuer: certIssuer ? certIssuer.textContent.trim() : 'unknown',
              certificate_url: certLink.href
            });
          }
        }
        return;
